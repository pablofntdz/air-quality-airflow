...
def prepare_stations(input_path, stations):
    df = load_data(input_path)
    df = build_features(df)
    base_path_model = Path(input_path).parent / "models"
    os.makedirs(base_path_model, exist_ok=True)
    base_path_inference = Path(input_path).parent / "inferece"
    os.makedirs(base_path_inference, exist_ok=True)
    station_paths = {}
    ...
    for station in stations:
        ...
        if df_station.empty:
            continue
        path = str(base_path_model / f"station_{station}.parquet")
...
def inference_pipeline():
    ...
    def task_load_data(input_path,station_id, station_paths):
        ...
        base_path = Path(input_path).parent / "inferece"
        inference_path = str(base_path / f"station_{station_id}.parquet")
    ...
    stations = [4, 8, 11, 16, 17, 27, 35, 36, 38, 39, 40, 47, 48, 50, 56, 57, 60]
    task_load_data.partial(input_path=input_path,station_paths=station_paths).expand(station_id=stations)
