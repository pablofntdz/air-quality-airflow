...
AIR_STATIONS_FILE = BASE_DIR / "data" / "AirQuality" / "stations" / "informacion_estaciones.csv"
TRAFFIC_FILE = BASE_DIR / "traffic_horas_exactas_v2.csv"
...
AIR_FILES = glob.glob(str(BASE_DIR / "air_quality/**/*.csv"), recursive=True)
METEO_FILES = glob.glob(str(BASE_DIR / "meteo/*.csv"), recursive=True)
...
def filter_traffic_df(df_traffic, df_traffic_est):
    df = pd.merge(df_traffic, df_traffic_est, on="id", how="left")
...
def calculate_distance_KDTree_traffic(df_traffic, df_air):
    df_traf = df_traffic[["id", "st_x", "st_y"]].drop_duplicates().reset_index(drop=True)
    df_air_loc = df_air[["estacion", "st_x", "st_y"]].drop_duplicates().reset_index(drop=True)
...
# ------------------------------------------------------------
def run_pipeline():
    df_air = pd.concat([pd.read_csv(f, sep=";") for f in AIR_FILES], ignore_index=True)
    df_air_stations = pd.read_csv(AIR_STATIONS_FILE, sep=";")

    df_traffic = pd.read_csv(TRAFFIC_FILE, sep=";")
    df_traffic_est = pd.read_csv(
    ...
    df_meteo = pd.concat([pd.read_csv(f, sep=";") for f in METEO_FILES], ignore_index=True)
    ...
    df_final = df_final.dropna(
        subset=[
            "valor", "VV", "DV", "T", "HR", "PB", "P",
            "intensidad_w", "ocupacion_w", "carga_w"
        ],
        how="any"
    )
