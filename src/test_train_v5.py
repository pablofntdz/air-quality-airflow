...
METEO_STATIONS_FILE = BASE_DIR / "300360-1-meteorologicos-estaciones-csv.csv"
url_est_meteo= "https://datos.madrid.es/dataset/300360-0-meteorologicos-estaciones/resource/300360-1-meteorologicos-estaciones-csv/download/300360-1-meteorologicos-estaciones-csv.csv"

TRAFFIC_STATIONS_FILE = BASE_DIR / "202468-212-intensidad-trafico-csv.csv"

url_meteo= "https://datos.madrid.es/dataset/300754-0-meteorologia-tiempo-real-acumula/resource/300754-3-meteorologia-tiempo-real-acumula/download/300754-3-meteorologia-tiempo-real-acumula.json"

url_air = "https://datos.madrid.es/dataset/300755-0-calidad-aire-tiempo-real-acumula/resource/300755-0-calidad-aire-tiempo-real-acumula/download/300755-0-calidad-aire-tiempo-real-acumula.csv"

url_traffic= "https://informo.madrid.es/informo/tmadrid/pm.xml"
url_traffic_est= "https://datos.madrid.es/dataset/202468-0-intensidad-trafico/resource/202468-284-intensidad-trafico/download/202468-284-intensidad-trafico.csv"
...
def filter_airquality_df(df_air, df_air_stations):
    ...
    df_air_stations = df_air_stations.drop(
        ["DIRECCION", "LONGITUD_ETRS89", "LATITUD_ETRS89", "COD_TIPO", "NOM_TIPO"],
        axis=1,
        errors="ignore"
    )
    df = pd.merge(
...
def run_pipeline():
    ...
    df_air = pd.read_csv(url_air,sep=";")
    df_air_stations = pd.read_csv(file_path_quality_stations,sep=";")
    headers = {'accept': 'application/xml;q=0.9, */*;q=0.8'}
    response=requests.get(url_traffic,headers = headers)
    print(response.status_code)

    df_traffic = pd.read_xml(url_traffic)
    df_traffic["fecha"] = pd.to_datetime(df_traffic.loc[0,"fecha_hora"],format="%d/%m/%Y %H:%M:%S")
    df_traffic = df_traffic.drop(["fecha_hora"],axis=1)
    df_traffic = df_traffic.drop(0,axis=0).reset_index(drop=True)
    df_traffic["fecha"] = pd.Timestamp.now().floor("h")
    df_traffic_est = pd.read_csv(TRAFFIC_STATIONS_FILE,sep=";",encoding="utf-8",encoding_errors="ignore")
    
    response=requests.get(url_meteo)
    print(response.status_code)
    data_meteo = response.json()
    df_meteo = pd.DataFrame(data_meteo["records"])
    df_est_meteo = pd.read_csv(METEO_STATIONS_FILE, sep=";")
    ...
    df_final.to_csv("dataset_final_hourly.csv", index=False)
