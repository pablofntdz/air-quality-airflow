from include.config.paths import RAW_DIR,DATA_HOURLY_DIR
import pandas as pd
import numpy as np
...
def load_raw_data(timestamp):
    ...
    key = f"year={dt:%Y}/month={dt:%m}/day={dt:%d}/"f"time={dt:%H%M}"
    
    raw_air = f"s3://{MINIO_BUCKET}/air/{key}/measure_air_{timestamp}.csv"
    raw_meteo = f"s3://{MINIO_BUCKET}/meteo/{key}/measure_meteo_{timestamp}.json"
    raw_traffic = f"s3://{MINIO_BUCKET}/traffic/{key}/measure_traffic_{timestamp}.xml"
    raw_air_station = f"s3://{MINIO_BUCKET}/air/{key}/stations_air_{timestamp}.csv"
    raw_meteo_station = f"s3://{MINIO_BUCKET}/meteo/{key}/stations_meteo_{timestamp}.csv"
    raw_traffic_station = f"s3://{MINIO_BUCKET}/traffic/{key}/stations_traffic_{timestamp}.csv"
    
    df_air = pd.read_csv(raw_air, sep=";", storage_options=STORAGE_OPTIONS)
    df_air_stations = pd.read_csv(raw_air_station, sep=";", storage_options=STORAGE_OPTIONS)

    df_traffic = pd.read_xml(raw_traffic, storage_options=STORAGE_OPTIONS)
    ...
    df_traffic_est = pd.read_csv(raw_traffic_station,sep=";",encoding="utf-8",encoding_errors="ignore",storage_options=STORAGE_OPTIONS)
    with pd.io.common.get_handle(raw_meteo, "r", storage_options=STORAGE_OPTIONS) as f:
        data = json.load(f.handle)          
    df_meteo = pd.DataFrame(data["records"])  
    df_est_meteo = pd.read_csv(raw_meteo_station, sep=";",storage_options=STORAGE_OPTIONS)
...
# Air Quality
# ------------------------------------------------------------
def filter_airquality_df(df_air, df_air_stations,historical=False):
...
# Meteo
# ------------------------------------------------------------
def filter_meteo_df(df_meteo, df_est,historical=False):
...
def save_processed_file(content: bytes, bucket: str, object_name: str) -> None:
    s3.put_object(
        Bucket=bucket,
        Key=object_name,
        Body=content
